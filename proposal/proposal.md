...
Ultrasound-based hand gesture classification has matured rapidly. A deep learning framework achieved 95.64% accuracy across 9 finger motions from forearm B-mode imaging [9], GPT Sonography (NeurIPS AIM-FM 2024) demonstrated that vision-language models can decode gestures from ultrasound images zero-shot without task-specific training [21], and IMEC's 2026 work achieved competitive classification accuracy with 87.5% fewer parameters through efficient architecture design [19]. The field is now shifting from discrete classification toward the harder problem of continuous regression. Bimbraw et al. (2023) predicted MCP joint angles with 7.35-degree RMSE from B-mode forearm ultrasound [8], wrist joint angles have been estimated continuously from ultrasound sequences [10], UltraGlove (SIGGRAPH Asia 2023) used finger-mounted MEMS ultrasonic sensors for full hand pose reconstruction [20], and the Nature Communications 2025 paper demonstrated user-generic VR hand control from a portable ultrasound armband — notably without per-user calibration [7].

The biomechanical foundation for these results lies in sonomyography — using ultrasound to image muscle dynamics directly. The JPO 2024 review establishes that ultrasound can distinguish deep individual muscles in ways fundamentally challenging for surface EMG [17], which underpins the hypothesis that ultrasound provides unique information for hand pose and force estimation. Proportional continuous control of hand and wrist movements has been demonstrated through sonomyographic interfaces [22, 23], and ProRuka achieved 6-DOF prosthetic control [18]. In clinical populations, ultrasound imaging detects hand motions in amputees with targeted muscle reinnervation at 83–99% accuracy [24], demonstrating that ultrasound captures motor intent even when surface signals are disrupted.

Wearable ultrasound hardware is maturing rapidly to support these capabilities. The WULPUS platform from ETH Zurich provides open-source A-mode wearable ultrasound at <30 mW power consumption [5]. UC San Diego's flexible wearable ultrasound monitors deep tissues during free movement, recognizing 13 degrees of freedom on the forearm [6]. Edge deployment is increasingly feasible: deep learning models achieve 92% gesture accuracy running on a Raspberry Pi from raw ultrasound [31], and unsupervised feature extraction achieves 96% gesture accuracy at only 16 mW [32]. However, most wearable systems use A-mode (1D signals) rather than B-mode (2D images), trading spatial resolution for form factor — and despite all this progress, no work achieves full continuous hand pose regression from wearable ultrasound at scale, and no study has systematically quantified ultrasound's unique contribution relative to and in combination with other sensing modalities through controlled cross-modal ablation.

### 7.2 Cross-Modal Sensor Fusion for Upper-Limb Sensing

Each sensing modality captures a different aspect of upper-limb movement, and a growing body of work explores their combination. Surface EMG remains the most widely studied modality: Meta's emg2pose benchmark (370 hours, 193 users) [3] set the standard for large-scale hand pose estimation from EMG, demonstrating both the scale achievable with wrist-worn sensors and the inherent limitations of electrical signals alone. Vision-based approaches like OpenCap [1] can estimate joint angles and even infer muscle activations through physics-based musculoskeletal simulation, but these inferences depend on model assumptions rather than direct observation. Wrist2Finger (UIST 2025) combines IMU and wrist-based EMG from a ring and watch to estimate finger positions and forces simultaneously [14], illustrating how complementary sensor placement improves estimation.

Fusion of ultrasound with other modalities is nascent but promising. ETH Zurich developed a sub-50 mW wearable that fuses 8-channel EMG with 4-channel A-mode ultrasound for hand-wrist kinematic tracking [11] — one of the first integrated EMG+US wearable systems. Zou et al. (2022) directly compared sEMG and A-mode ultrasound for hand force estimation in a multimodal fusion model [25], finding complementary strengths. A comprehensive review of sEMG-IMU fusion covering 181 papers identifies the lack of standardized cross-modal benchmarks as a major gap in the field [2]. Critically, no existing study includes ultrasound in a systematic multi-sensor ablation that quantifies each modality's unique and complementary contribution across all meaningful combinations — the central objective of UltraPose (Year 1).

### 7.3 Multimodal Reasoning Benchmarks and Datasets

MMTouch [4] pioneered multimodal reasoning benchmarks for physical interaction, pairing multi-view vision with multi-tactile sensing across 82 objects, 13 manipulation types, and 37,571 QA pairs. Its key finding — that multi-tactile supervision yields the largest per-modality gain (+18.9%) — demonstrates the power of rich, synchronized multi-sensor annotations for training models to reason about manipulation. A broader ecosystem of visual-tactile learning reinforces this paradigm: NeuralFeels (Science Robotics 2024) combines vision and touch for robotic in-hand manipulation [28], GenForce (Nature Communications 2026) enables transferable force sensing across diverse tactile sensor types [29], and the ManiSkill-ViTac challenge benchmarks vision-tactile manipulation policies [30].

However, all these benchmarks and datasets capture only the *external* side of physical interaction — surface pressure, visual appearance, contact geometry. No reasoning benchmark includes the *internal* modality: the muscle dynamics and tendon mechanics that cause the observed manipulation. Existing ultrasound-inclusive datasets are limited to lower-limb movements: K2MUSE (2025) combines A-mode ultrasound, sEMG, and Vicon motion capture for 30 subjects across 20 lower-limb ambulation conditions [38], and AnkleImage (Scientific Data 2024) provides ultrafast B-mode ultrasound of ankle muscles with EMG and kinematics [39]. No dataset pairs forearm ultrasound with vision, tactile, and EMG data during hand manipulation — the gap our Year 2 benchmark directly addresses.

### 7.4 Foundation Models and Self-Supervised Learning for Movement Understanding

Foundation models are beginning to transform movement science, but operate exclusively on external signals. GaitDynamics (Stanford, Nature BME 2026) built a diffusion-based generative model on large-scale gait data [12], and BiomechGPT (Northwestern, 2025) combines tokenized movement sequences with language for clinically relevant motion tasks [13] — both powerful approaches that nonetheless rely entirely on external kinematics (joint angles, body trajectories). Multi-modal wearable foundation models are emerging: MASTER (IMWUT 2025) handles multiple sensor modalities for human activity recognition [26], and NormWear (2024) pretrains on 14,943 hours of physiological signals including EMG and accelerometer data [27]. Neither incorporates ultrasound.

On the ultrasound side, self-supervised learning methods have been developed for clinical imaging contexts: UltraFedFM applies federated self-supervised pretraining to build ultrasound foundation models across institutions [15], and self-supervised temporal reconstruction has been used for muscle atrophy evaluation from ultrasound sequences [16]. However, these methods target diagnostic clinical ultrasound (organ imaging, pathology detection) rather than wearable movement sensing. No foundation model bridges these domains by incorporating ultrasound as a modality for understanding human movement — a gap that our multimodal models (Years 1–3) and self-supervised pretraining strategy (Year 2) begin to address.

### 7.5 AI for Physical Training, Rehabilitation, and Sports Biomechanics

AI tools for analyzing muscle and tendon dynamics from ultrasound exist but operate offline. DL_Track_US automates muscle fascicle tracking (pennation angle, fascicle length) from B-mode ultrasound images [33], and StrainNet estimates tendon deformation from ultrasound sequences [34] — both process recorded data after the session rather than providing real-time feedback. Clinical evidence supports the value of real-time ultrasound feedback: wearable ultrasound visual biofeedback has been shown to significantly improve muscle contraction training outcomes compared to training without internal visualization [35], and ultrasound assessment of muscle and tendon properties is increasingly used in elite sports to inform training response and injury risk [36].

The broader AI-for-sports ecosystem is growing but relies on external observation. A scoping review of AI in sports biomechanics [37] and work on AI-driven medical image analysis for sports injury prevention [40] both demonstrate expanding capabilities, yet all systems operate from external kinematics or post-hoc imaging — none uses ultrasound to observe internal muscle recruitment in real time during athletic performance. The gap is twofold: no AI system uses ultrasound to evaluate movement correctness at the internal muscle level, and no system proactively guides training by observing internal muscle dynamics and adapting feedback in real time. Year 3's agentic training system addresses both.
